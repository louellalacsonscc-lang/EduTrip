...
function loadUserProfile() {
    if (currentUser) {
        ...
        const birthdateEl = document.getElementById('user-birthdate');
        ...
        if (courseEl) courseEl.textContent = currentUser.course || 'Not provided';
        if (sectionEl) sectionEl.textContent = currentUser.section || 'Not provided';
        if (yearEl) yearEl.textContent = currentUser.year || 'Not provided';
        if (birthdateEl) birthdateEl.textContent = currentUser.birthdate || 'Not provided';
        if (ageEl) ageEl.textContent = currentUser.age || 'Not provided';
        if (sexEl) sexEl.textContent = currentUser.sex || 'Not provided';
    }
...
function populateProfileModal() {
    ...
    if (course) course.value = currentUser.course || '';
    if (section) section.value = currentUser.section || '';
    if (year) year.value = currentUser.year || '';
    if (birthdate) birthdate.value = currentUser.birthdate || '';
    if (age) age.value = currentUser.age || '';
    if (sex) sex.value = currentUser.sex || '';
}
...
function setupProfileModal() {
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', submitProfileCompleteForm);
    }
}
    ...
    try {
        ...
        eventsList.innerHTML = events.map(event => `
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-blue-500/30 transition-all duration-300 hover:shadow-lg">
                ${event.image_url ? `<div class="mb-4 overflow-hidden rounded-xl"><img src="${event.image_url}" alt="${event.title || 'Event image'}" class="w-full h-48 object-cover" /></div>` : ''}
        ...
        if (notificationPage) {
            ...
            notificationsList.innerHTML = notifications.map(notification => `
                <div class="bg-gray-900 border ${notification.is_read ? 'border-gray-800' : 'border-blue-800/50'} rounded-xl p-5 cursor-pointer hover:bg-gray-800/50 transition-colors"
                    ...
                    <span class="inline-block mt-2 px-2 py-1 text-xs font-medium rounded ${notification.type === 'warning' ? 'bg-yellow-900/30 text-yellow-400 border border-yellow-800' :
            ...
            return `
                <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-blue-500/30 transition-colors">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        <span class="px-3 py-1 rounded-full text-sm font-medium ${reg.status === 'approved' ? 'bg-green-900/30 text-green-400 border border-green-800' :
